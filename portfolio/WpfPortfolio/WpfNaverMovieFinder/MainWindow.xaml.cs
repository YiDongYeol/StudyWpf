...
namespace WpfNaverMovieFinder
{
    ...
    {
        private bool IsFavorite = false;    // 네이버 API로 검색한건지, 즐겨찾기 DB에서 가져온건지 확인할 값
        ...
        private void btnNaverMovie_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (IsFavorite)
            {
                System.Diagnostics.Process.Start((grdResult.SelectedItem as TblFavoriteMovies).Link);
            }
            else
            {
                System.Diagnostics.Process.Start((grdResult.SelectedItem as MovieItem).Link);
            }
        }
    }
}
